...
export default function LegalNoticeFR() {
  ...
  return (
    <div className="min-h-screen bg-memopyk-cream">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 mt-20">
        <div className="bg-memopyk-cream rounded-lg shadow-lg p-8 border border-memopyk-blue/10">
          <h1 className="text-3xl font-bold text-memopyk-navy font-poppins mb-8">{legalContent.fr.legalNotice.title}</h1>
          
          {legalContent.fr.legalNotice.sections.map((section: any, index: number) => (
            <section key={index} className="mb-8">
              <h2 className="text-xl font-semibold text-memopyk-navy mb-4 font-poppins">{section.title}</h2>
              <div className="text-memopyk-blue space-y-1 legal-content">
                {section.content.map((item: any, itemIndex: number) => (
                  <p key={itemIndex} dangerouslySetInnerHTML={{ __html: item.value }}></p>
                ))}
              </div>
            </section>
          ))}
          ...
          <p className="text-sm text-memopyk-blue-light mt-8">
            Dernière mise à jour: {legalContent.fr.legalNotice.lastUpdated}
          </p>
        </div>
      </main>
